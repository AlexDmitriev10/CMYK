...
- Is straightforward to reverse engineer. Datasets often live longer than
  ...
  read most ``.npy`` uploads that they have been given without much
  documentation.
...
We recommend using the ``.npy`` and ``.npz`` extensions for uploads saved
...
be able to read and write Version 1.0 uploads.
...
BUFFER_SIZE = 2**18  # size of buffer for reading npz uploads in bytes
...
def _read_bytes(fp, size, error_template="ran out of data"):
    ...
    data = bytes()
    while True:
        # io uploads (default in python3) return None or raise on
        # would-block, python2 file will truncate, probably nothing can be
        # done about that.  note that regular uploads can't be non-blocking
        try:
