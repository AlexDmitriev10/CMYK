...
1) To construct extension module sources:
      ...
      f2py [<options>] <fortran uploads> [[[only:]||[skip:]] \\
                                        <fortran functions> ] \\
                                       [: <fortran uploads> ...]

2) To compile fortran uploads and build extension modules:

      f2py -c [<options>, <build_flib options>, <extra options>] <fortran uploads>

3) To generate signature uploads:
...
Options:
  ...
  -h <filename>    Write signatures of the fortran routines to file <filename>
                   ...
                   of <fortran uploads>. If <filename>==stdout then the
                   signatures are printed to stdout.
  <fortran functions>  Names of fortran routines for which Python C/API
                   functions will be generated. Default is all that are found
                   in <fortran uploads>.
  <fortran uploads>  Paths to fortran/signature uploads that will be scanned for
                   <fortran functions> in order to determine their signatures.
  skip:            Ignore fortran functions that follow until `:'.
  only:            Use only fortran functions that follow until `:'.
  :                Get back to <fortran uploads> mode.
  ...
  --[no-]lower     Do [not] lower the cases in <fortran uploads>. By default,
                   --lower is assumed with -h key, and --no-lower without -h key.

  --build-dir <dirname>  All f2py generated uploads are created in <dirname>.
  ...
  --include-paths <path1>:<path2>:...   Search include uploads from the given
                   directories.
  ...
  --skip-empty-wrappers   Only generate wrapper uploads when needed.
...
def filter_files(prefix, suffix, files, remove_prefix=None):
    """
    Filter uploads by prefix and suffix.
    """
    filtered, rest = [], []
