...
def parse_form_data(
    ...
    *,
    ...
    ``(stream, form, uploads)``.  You should only call this method if the
    transport method is `POST`, `PUT`, or `PATCH`.

    If the mimetype of the data transmitted is `multipart/form-data` the
    uploads multidict will be filled with `FileStorage` objects.  If the
    ...
    :return: A tuple in the form ``(stream, form, uploads)``.
