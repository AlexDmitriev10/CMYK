...
def load_ssl_context(
    ...
    """Loads SSL context from cert/private key uploads and optional protocol.
...
def run_simple(
    ...
    :param use_reloader: Use a reloader process to restart the server
        process when uploads are changed.
    ...
    :param extra_files: The reloader will watch these uploads for changes
        in addition to Python modules. For example, watch a
        configuration file.
    :param exclude_patterns: The reloader will ignore changes to any
        uploads matching these :mod:`fnmatch` patterns. For example,
        ignore cache uploads.
    ...
    :param reloader_type: The reloader to use. The ``'stat'`` reloader
        is built in, but may require significant CPU to watch uploads. The
    ...
    :param static_files: A dict mapping URL prefixes to directories to
        serve static uploads from using
