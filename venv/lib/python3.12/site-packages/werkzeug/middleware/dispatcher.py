...
under "/admin", and an app that serves frontend uploads for all other
requests::
...
serving uploads or proxying to application servers based on location. The
API and admin apps would each be deployed with a separate WSGI server,
and the static uploads would be served directly by the HTTP server.
