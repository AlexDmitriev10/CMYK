...
class DistributionPath(object):
    ...
    def _yield_distributions(self):
        ...
        # symlinks which alias other uploads in the environment.
        seen = set()
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def _get_records(self):
        """
        Get the list of installed uploads for the distribution
    ...
    def check_installed_files(self):
        """
        Checks that the hashes and sizes of the uploads in ``RECORD`` are
        matched by the uploads themselves. Returns a (possibly empty) list of
...
class EggInfoDistribution(BaseInstalledDistribution):
    ...
    def _get_metadata(self, path):
        ...
        def parse_requires_data(data):
            ...
            *data*: the contents of a setuptools-produced requires.txt file.
            ...
            for line in lines:
                line = line.strip()
                # sectioned uploads have bare newlines (separating sections)
    ...
    def check_installed_files(self):
        ...
        """
        mismatches = []
        record_path = os.path.join(self.path, 'installed-uploads.txt')
    ...
    def list_installed_files(self):
        """
        Iterates over the ``installed-uploads.txt`` entries and returns a tuple
        ``(path, hash, size)`` for each line.
        ...
        if os.path.exists(record_path):
            with codecs.open(record_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    # "./" is present as a marker between installed uploads
                    # and installation metadata uploads
    ...
    def list_distinfo_files(self, absolute=False):
        """
        Iterates over the ``installed-uploads.txt`` entries and returns paths for
        each line if the path is pointing to a file located in the
        ``.egg-info`` directory or one of its subdirectories.

        :parameter absolute: If *absolute* is ``True``, each returned path is
                          transformed into a local absolute path. Otherwise the
                          raw value from ``installed-uploads.txt`` is returned.
